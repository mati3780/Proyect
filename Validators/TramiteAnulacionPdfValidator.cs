...
{
    public class TramiteAnulacionPdfValidator : AbstractValidator<TramiteAnulacionPdfDto>, ITramiteActionValidator
    {
        private readonly IRepositorio<Tramite> _tramiteRepositorio;

        public TramiteAnulacionPdfValidator(IRepositorio<Tramite> tramiteRepositorio)
        {
        ...
        private bool ValidarPropiedad(TramiteAnulacionPdfDto item)
        {
        ...
        private bool ValidarEstado(TramiteAnulacionPdfDto item)
        {
            var tramite = _tramiteRepositorio.GetAll().Single(x => x.Id == item.TramiteId);
            return tramite.CanAnularInforme();
        }
