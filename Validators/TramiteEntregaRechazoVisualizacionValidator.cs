...
{
    public class TramiteEntregaRechazoVisualizacionValidator : AbstractValidator<TramiteEntregaRechazoVisualizacionDto>, ITramiteActionValidator
    {
        private readonly IRepositorio<Tramite> _tramiteRepositorio;

        public TramiteEntregaRechazoVisualizacionValidator(IRepositorio<Tramite> tramiteRepositorio)
        {
        ...
        private bool ValidarPropiedad(TramiteEntregaRechazoVisualizacionDto item)
        {
        ...
        private bool ValidarEstado(TramiteEntregaRechazoVisualizacionDto item)
        {
            var tramite = _tramiteRepositorio.GetAll().Single(x => x.Id == item.TramiteId);
            return tramite.CanEntregaRechazar();
        }
