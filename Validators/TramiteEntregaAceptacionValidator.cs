...
{
    public class TramiteEntregaAceptacionValidator : AbstractValidator<TramiteEntregaAceptacionDto>, ITramiteActionValidator
    {
        private readonly IRepositorio<Tramite> _tramiteRepositorio;

        public TramiteEntregaAceptacionValidator(IRepositorio<Tramite> tramiteRepositorio)
        {
        ...
        private bool ValidarPropiedad(TramiteEntregaAceptacionDto item)
        {
        ...
        private bool ValidarEstado(TramiteEntregaAceptacionDto item)
        {
            var tramite = _tramiteRepositorio.GetAll().Single(x => x.Id == item.TramiteId);
            return tramite.CanAceptarEntrega();
        }
