...
{
    public class TramiteVerificacionValidator : AbstractValidator<TramiteVerificacionDto>, ITramiteActionValidator
    {
        private readonly IRepositorio<Tramite> _tramiteRepositorio;

        public TramiteVerificacionValidator(IRepositorio<Tramite> tramiteRepositorio)
        {
        ...
        private bool ValidarPropiedad(TramiteVerificacionDto item)
        {
        ...
        private bool ValidarEstado(TramiteVerificacionDto item)
        {
            var tramite = _tramiteRepositorio.GetAll().Single(x => x.Id == item.TramiteId);
            return tramite.CanVerificarInforme();
        }
