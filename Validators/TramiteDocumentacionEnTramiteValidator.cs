...
{
    class TramiteDocumentacionEnTramiteValidator : AbstractValidator<DocumentacionEnTramiteDto>, ITramiteActionValidator
    {
        private readonly IRepositorio<Tramite> _tramiteRepositorio;

        public TramiteDocumentacionEnTramiteValidator(IRepositorio<Tramite> tramiteRepositorio)
        {
        ...
        private bool ValidarPropiedad(DocumentacionEnTramiteDto item)
        {
        ...
        private bool ValidarEstado(DocumentacionEnTramiteDto item)
        {
            var tramite = _tramiteRepositorio.GetAll().Single(x => x.Id == item.TramiteId);
            return tramite.CanDocumentacionEnTramite();
        }
