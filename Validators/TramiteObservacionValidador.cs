...
{
    public class TramiteObservacionValidador : AbstractValidator<TramiteObservacionDto>, ITramiteActionValidator
    {
        private readonly IRepositorio<Tramite> _tramiteRepositorio;

        public TramiteObservacionValidador(IRepositorio<Tramite> tramiteRepositorio)
        {
        ...
        private bool ValidarPropiedad(TramiteObservacionDto item)
        {
        ...
        private bool ValidarEstado(TramiteObservacionDto item)
        {
            var tramite = _tramiteRepositorio.GetAll().Single(x => x.Id == item.TramiteId);
            return tramite.CanObservarTramite();
        }
